 ...
 * @author Reg
 */
public class BoatSlipDAO extends CoreDAOImpl	{
	/**
	 * Creates a new instance of BoatSlipDAO
	 */
	public BoatSlipDAO() { this(CoreDAO.DRIVER_NAME, CoreDAO.URL, CoreDAO.USER, CoreDAO.PASSWORD);		}
	 ...
	 */
	public BoatSlipDAO(String drivername,
						String url,
						String user,
						String password)	{
	...
	public void dbInsert(Object data, String insertStm) throws DAOSysException		{
		BoatSlipModel model = (BoatSlipModel) data;
		...
		try	{
			...
			preparedStm.setString(1, model.getBoatPK().getRegistrationNo());
			preparedStm.setString(2, model.getSlipPK().getNumber());
			preparedStm.executeUpdate();

		}	catch (SQLException sex)	{
			throw new DAOSysException("Error adding BoatSlip <" + model.getBoatPK() + ":" + model.getSlipPK() + "> " + sex.getMessage());
	...
	public Object dbSelectByPrimaryKey(Object primarykey)
		...
		return dbSelectByPrimaryKey(primarykey, new BoatSlipModel());
	}
	...
	public Object dbSelectByPrimaryKey(Object primarykey, String selectStm, Object model)
		...
		BoatSlipPK pk = (BoatSlipPK) primarykey;
		Connection connection = null;
		PreparedStatement preparedStm = null;
		ResultSet rs = null;
		boolean result = false;
		BoatSlipModel persistenceModel = (BoatSlipModel) model;

		try	{
			connection = connectToDB();
			preparedStm = connection.prepareStatement(selectStm);
			preparedStm.setString(1, pk.getBoatPK().getRegistrationNo());
			preparedStm.setString(2, pk.getSlipPK().getNumber());
			rs = preparedStm.executeQuery();

			result = rs.next();
			if (result)	{
				persistenceModel.setPrimarykey(new BoatSlipPK(
																new BoatPK(rs.getString(1)),
	...
	public Collection dbSelectAll()	throws DAOSysException {
		String selectStm = "SELECT DISTINCT boatRegistrationNo "
								+ "FROM " + "BoatSlip";
	...
	public Collection dbSelectAll(String selectStm)	throws DAOSysException {
		...
		PreparedStatement preparedStm = null;
		ResultSet rs = null;
		ArrayList<BoatSlipPK> list = null;
		...
		try	{
			...
			list = new ArrayList<BoatSlipPK>();
			while (rs.next())	{
				list.add(new BoatSlipPK(
									new BoatPK(rs.getString(1)),
									new SlipPK(rs.getString(2)))
								);
			}
	...
	public void dbUpdate(Object data)	throws DAOSysException	{
		String updateStm = "UPDATE " + "BoatSlip"
								+ " SET "
								+ "boatRegistrationNo = ? "
								+ "slipNumber = ? "
								+ " WHERE boatRegistrationNo = ?"
	...
	public int dbRemove(Object primarykey)	throws DAOSysException	{
		String deleteStm = "DELETE FROM " + "BoatSlip"
								+ " WHERE boatRegistrationNo = ?"
								+ " AND slipNumber = ?";
	...
	public int dbCountTotalEntities()	throws DAOSysException	{
		String selectStm = "SELECT DISTINCT boatRegistrationNo FROM "
							+ "BoatSlip";
		Connection connection = null;
		PreparedStatement preparedStm = null;
		ResultSet rs = null;
		...
		try	{
			...
			throw new DAOSysException(
						"dbCountTotalBoatSlips() SQL Exception\n"
						+ sex.getMessage());
