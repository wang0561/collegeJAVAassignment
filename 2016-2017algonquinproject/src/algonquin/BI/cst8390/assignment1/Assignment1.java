package algonquin.BI.cst8390.assignment1;
...
public class Assignment1 extends Application {
	...
	private long getNumberOfHouse(double low, double high, ArrayList<Mydata> list) {
		...
		for (int i = 0; i < listOfMydata.size(); i++) {
			listOfValue.add(listOfMydata.get(i).getCurrentValue());
		}

		count = listOfValue.stream().filter(i -> i < high).filter(i -> i >= low).count();
	...
	private double AveHousePrice(ArrayList<Mydata> list) {
		...
		for (int i = 0; i < list.size(); i++)
			sum += list.get(i).getCurrentValue();
	...
	private double totalValueChanged(ArrayList<Mydata> list) {
		long sum = 0;
		for (int i = 0; i < list.size(); i++) {
			sum += list.get(i).getCurrentValue() - list.get(i).getPreValue();
		}
		return sum;
	}
	 ...
	 * 
	 */
	protected ObservableList<StreetAverage> getAveViewByStreet(TreeMap<String, ArrayList<Double>> map) {
		double mean = 0;
		double sum = 0;
		...
		while (it.hasNext()) {
			Entry<String, ArrayList<Double>> me = it.next();
			String key = me.getKey();
			ArrayList<Double> value = me.getValue();
	...
	protected void openOtherStage() {
		...
		// create a tree map to store the information of street name and average
		// value
		TreeMap<String, ArrayList<Double>> tp = new TreeMap<>();
		for (Mydata mydata : listOfMydata) {
			String name = mydata.getStreetName();
			double value = mydata.getCurrentValue();
			if (name != null) {
				if (tp.containsKey(name)) {
					tp.get(name).add(value);
		...
		tableStreet.setItems(getAveViewByStreet(tp));
		...
		tableStreet.setOnMouseClicked(new EventHandler<MouseEvent>() {
			@Override
			public void handle(MouseEvent mouseEvent) {
				if(mouseEvent.getButton().equals(MouseButton.PRIMARY)){
					if(mouseEvent.getClickCount() == 2){
						...
						bp.setCenter(text);
						TableView<PostCodeAverage> table = getTableByPostalCode(data);
						root.setRight(table);
						} catch (Exception e1) {
							e1.getMessage();
						}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private TableView<PostCodeAverage> getTableByPostalCode(String data) {
		// TODO Auto-generated method stub
		// create a variable for store the street name
		String streetName = data;
		// create a list for deposit the post code from the same street name

		TreeMap<String, ArrayList<Double>> mapOfPostCodeAve = new TreeMap<String, ArrayList<Double>>();

		for (Mydata mydata : listOfMydata) {
			String postcode = mydata.getPostalCode();
			double value = mydata.getCurrentValue();
			if (mydata.getStreetName().equals(streetName) ) {
				if (mapOfPostCodeAve.containsKey(postcode)) {
					mapOfPostCodeAve.get(postcode).add(value);

				} else {
					ArrayList<Double> listOfHouseValue = new ArrayList<>();
					listOfHouseValue.add(value);
					mapOfPostCodeAve.put(postcode, listOfHouseValue);
				}
		...
		aveCol.setCellValueFactory(new PropertyValueFactory<>("average"));
		aveCol.setStyle("-fx-alignment: CENTER;");
		tablePost.setItems(getAveViewByPostalCode(mapOfPostCodeAve));
	 ...
	 */
	protected ObservableList<PostCodeAverage> getAveViewByPostalCode(TreeMap<String, ArrayList<Double>> map) {
		double mean = 0;
		double sum = 0;
