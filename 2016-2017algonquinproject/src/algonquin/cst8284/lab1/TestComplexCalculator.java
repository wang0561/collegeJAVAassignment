...
public class TestComplexCalculator {
	...
	public static void main(String[] args) {
		...
		while (true) { // loop until 'Q' entered by user, below
			Complex c1 = TestComplexCalculator.getComplexNumber(); // read in
																	// the 1st
																	// complex
																	// number
			Complex c2 = TestComplexCalculator.getComplexNumber(); // read in
																	// the 2nd
																	// complex
																	// number
			calc = new ComplexCalculator(c1, c2); // perform the calculation
			System.out.println("The result is: " + calc.toString()); // output
																		// the
																		// result
			System.out.println("Do you wish to continue? Enter Q to quit or anything else to continue");
			if (input.next().charAt(0) == 'Q')
				break; // press 'Q' to quit, otherwise loop
		}
	}
	...
	private static Complex getComplexNumber() {
		...
		System.out.println("Enter the " + (counter % 2 == 0 ? "second" : "first") + " Complex number");
		...
		default:
			...
			System.exit(0);
			return (new Complex()); // not used, but required: JVM expects a
									// Complex return type
		}
	}
	...
	private static int displayTypeInputMenu() {
		System.out.printf("%s\n\t%s\n\t%s\n\t%s", "Enter the format of the complex number to be entered:",
				"1. A single string", "2. 2 strings", "3. 2 integer values\n");
		counter++;
